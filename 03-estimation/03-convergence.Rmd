...
file_wikipedia <-
  "../01-data-sources/02-wikipedia/wikipedia-data/01-wp-data-json.zip"
...
```{r}
params <- setNames(
  c("delta", "lambda_1", "lambda_2", "beta", "x_1", "x_2"),
  nm = c("delta", "lambda_1", "lambda_2", "beta", "x_1", "x_2")
)

if (wp_model == 2) {

  params <- c(setNames(c("gamma", "tau"), nm = c("gamma", "tau")), params)
...
samples <- rstan::extract(stan_out, pars = params)
```
...
get_df <- function(var) {
  ...
  dt <- data.frame(
    maxmin = round(sapply(param_stat, max, na.rm = TRUE), digits),
    quant = t(
      round(sapply(param_stat, quantile, quantile_select, na.rm = TRUE), digits)
    ),
    mean = round(sapply(param_stat, mean, na.rm = TRUE), digits),
    N_par = sapply(param_stat, length)
  )
  if (var == "n_eff") {

    dt$maxmin <- round(sapply(param_stat, min, na.rm = TRUE), digits)
...
Confirming $\bar{\hat{x}_1} = \bar{\hat{x}_2} = 0$

```{r}
round(mean(samples$x_1), 3)
round(mean(samples$x_2), 3)
```

Confirming $\text{SD}(\hat{x}_1) = \text{SD}(\hat{x}_2) = 1$

```{r}
round(sd(samples$x_2), 3)
round(sd(samples$x_2), 3)
```
...
```{r}
plot(density(apply(samples$x_1, 2, median)), main = "1st dim")
plot(density(apply(samples$x_2, 2, median)), main = "2nd dim")
```
